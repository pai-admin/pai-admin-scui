...
export default {
	...
	methods: {
		...
		async getGroup(){
			...
			this.groups = this.$TOOL.makeTreeData(res.data, 0, "deptId");
		},
		//表单提交方法
		async submit(){
			// 校验登录
			...
			this.isSaveing = true;
			const res = this.form.deptId ? await this.$API.system.dept.edit(this.form) : await this.$API.system.dept.add(this.form);
